...
# CAPTCHA image link (direct download link from OneDrive with cache-buster to ensure the latest image)
...
def initialize_solutions():
    if "submitted_answers" not in st.session_state:
        st.session_state["submitted_answers"] = pd.DataFrame(columns=["Solution", "Date & Time"])
...
def submit_solution():
    if st.session_state["captcha_input"].strip():
        ...
        st.session_state["submitted_answers"] = pd.concat(
            [new_row, st.session_state["submitted_answers"]], ignore_index=True
        )
